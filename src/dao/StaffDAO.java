...
public class StaffDAO extends DAO {
     ...
     * @return
     */
    public ArrayList<Doctor> getDoctors(int id) throws SQLException {
...
//                        "where tbdepartment.id = ? and tbstaff.id = tbdoctor.id and tbdoctor.iddepartment = tbdepartment.id and tbdoctor.idlevel = tblevel.id;";
        String sql = "select * from tbstaff, tbdoctor, tblevel, tbdepartment, tbrecord "
                + "where tbrecord.iddoctor = ? and tbstaff.id = tbdoctor.id and tbdoctor.iddepartment = tbdepartment.id and tbdoctor.idlevel = tblevel.id;";
        PreparedStatement ps = con.prepareStatement(sql);
        ps.setInt(1, id);
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
